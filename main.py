...
def main():
    ...
    try:
        x_alvo = float(input("Digite o horário alvo: "))
        if not (10 <= x_alvo <= 16):
    ...
    if opcao_metodo in ['1', '2']:
        ...
        print("\nGerando Gráfico 1 (Com dados reais)")
        plt.figure(figsize=(12, 7))
        ...
        # Pontos e Alvos
        plt.scatter(x_treino, y_treino, color='blue', s=40, zorder=4)
        plt.scatter(x_quad, y_quad, color='red', s=100, marker='o', zorder=5)
        ...
        plt.title(f"Comparativo: Reais vs Modelos - {ticker}", fontsize=14)
        plt.xlabel("Hora do Dia")
        plt.ylabel("Preço (R$)")
        plt.legend()
        plt.grid(True, linestyle='--', alpha=0.6)
        ...
        arquivo_1 = f"grafico_{ticker}_comparativo_COM_REAL.png"
        plt.savefig(arquivo_1, dpi=300)
        plt.close() # Limpa o gráfico para começar o próximo
        print(f"A imagem foi salva como: {arquivo_1}")

        print("Gerando Gráfico 2 (Somente polinômios)")
        plt.figure(figsize=(12, 7))
        ...
        # Pontos e Alvos (Mantidos para referência visual da curva)
        ...
        plt.title(f"Comparativo: Apenas Modelos Matemáticos - {ticker}", fontsize=14)
        plt.xlabel("Hora do Dia")
        plt.ylabel("Preço Estimado (R$)")
        plt.legend()
        plt.grid(True, linestyle='--', alpha=0.6)
        plt.xticks(range(10, 17))
        
        arquivo_2 = f"grafico_{ticker}_comparativo_SEM_REAL.png"
        plt.savefig(arquivo_2, dpi=300)
        plt.close()
        print(f"A imagem foi salva como: {arquivo_2}")
